...
export async function listActiveSkills(db: Database): Promise<SkillInfo[]> {
  const filesystem = await discoverSkills();
  const activeNames = new Set(listDbSkills(db, "active").map((row) => row.name));
  return filesystem.filter((s) => activeNames.has(s.name));
}

export async function listCandidateSkills(db: Database): Promise<SkillInfo[]> {
  const filesystem = await discoverSkills();
