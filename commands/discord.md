...
3. **If $ARGUMENTS contains "clear"**: Delete `.claude/hermes/session.json` to reset the global session. Confirm to the user. The next run from any source (heartbeat, cron, Telegram, or Discord) will create a fresh session.

4. **Running**: Check if the daemon is running by reading `.claude/hermes/daemon.pid`. The Discord bot runs in-process with the daemon when a token is configured.
